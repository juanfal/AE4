...
def doAssociationQueueAndConsume():
    ...
    for iCell in range(gNumberOfCells):
        ...
        q = iter(queue)
        waste = 0
        while (not gArgs["swallow"] and rsrc >= gMinEating and q.toIter() > 0) or (waste == 0 and q.toIter() > 0):
            couple = next(q)
            iOrig = couple[0]
            iDest = couple[1]
            ...
            if iDest < 0:  # iOrig is INDIVIDUAL or RECIPIENT
                ...
                toEat = dirFit
                waste = toEat
                #toEat = dirFit + abs(indirFit) # taking advantage of the indirFit for himself
...
def saveWorld(genNumber):
    ...
    with open((fname+ ("%04d" % genNumber) + ext) , 'w') as outfile:
        for iCell in range(gNumberOfCells):
...
def checkConf(conf):
    ...
    if "DistType" in conf and conf["DistType"] not in "rhn":
        problems += "DistType global value must end in r, n or h. You gave %s\n" % conf["DistType"]
    ...
    for i in range(longListSpecies):
        ...
        if "DistType" in confi and confi["DistType"] not in "rhn":
            problems += "DistType for species %d must end in r, n or h\n" % i
        ...
        if len(diffKeys):
            problems += """\
Some essential item(s) of species '%s' (number %d) is not defined

    Check syntax: %s""" % (theId, i, str(diffKeys))

    # pprint(mustBeDefined)
    for theID in set(mustBeDefined):
        if iFrom_id(theID) == -1:
            problems += "The species '%s' should be defined as it is a potential group\n" % theID
...
def defineAndGetCommandLineArgs():
    ...
    theArgParser = argparse.ArgumentParser(description=f"""
...
def telegramNotificationFinished(message):
    import subprocess
    ...
    subprocess.call(["curl",
                     ...
                     "bot"+bot_token+"/sendMessage?chat_id="+chat_id+"&text="+
                     quote(f"From {host}.\n\nHa acabado este:\n\n{message}")
                    ], stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL)

def telegramNotificationStarting(message):
    import subprocess
    from urllib.parse import quote
...
def timeRecording(tNow, nGens, totGens, td, fromArgs):
    if not os.path.isfile(TIMERECORDFILE):
        with open(TIMERECORDFILE, "w") as f:
    ...
    # hours += days*24
    with open(TIMERECORDFILE, "a") as f:
...
def build_defaultjson(inDir):
    with open(os.path.join(inDir, 'defaultInit.json'), 'w') as f:
        f.write("""{
    "NumberOfCells": 15,
...
if gToSaveExcel:
    ...
    if os.path.isfile(gExcelOut):
        ...
        while os.path.isfile(baseName+'.xlsx'):
            baseName = "%s_%d" % (baseName, i)
            i += 1
        gExcelOut = baseName + '.xlsx'

if gConf['redirectStdout']:
    sys.stdout=open(os.path.join(gOutDir, gOutFNameBase + "_stdout.txt"),"a")
